...
resource "azurerm_api_management_api_operation" "security_poc_api_echo_appreader_rolecheck" {
    ...
    operation_id        = "apim-sec-poc-api-echo-appreader-role-check"
    
    display_name        = "Echo with AppReader Role Check"
    method              = "GET"
    url_template        = "/echo/appreader/rolecheck"
...
#Operation Policy
resource "azurerm_api_management_api_operation_policy" "security_poc_api_echo_appreader_rolecheck" {
    resource_group_name = data.azurerm_resource_group.apim_resource_group.name
    api_management_name = data.azurerm_api_management.apim_instance.name
    api_name            = azurerm_api_management_api.security_poc_api.name   
    operation_id        = azurerm_api_management_api_operation.security_poc_api_echo_appreader_rolecheck.operation_id
        ...
        <validate-jwt header-name="Authorization" failed-validation-httpcode="401" require-scheme="Bearer"
            ...
            <required-claims>
              <claim name="roles" match="any">
                <value>${local.apim_security_roles_names.AppReader}</value>
              </claim>
            </required-claims>
        </validate-jwt>
